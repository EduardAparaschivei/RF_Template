...
public class SupervisedLearningSet  extends UnsupervisedLearningSet {
    ...
    public double[] calculateWeightsValues(){
    	Map<Pattern, Pair<Double, Integer>> patternsMap = getPatternsMapFromInitialSet(X,iClass);
        n = patternsMap.size();
        X = new double[n][p];
        iClass = new int[n];
        ...
        for(var entry:patternsMap.entrySet()){
            X[i] = entry.getKey().getPatternValues();
            f[i] = entry.getValue().getA();
            iClass[i++] = entry.getValue().getB();
        }
